...
[TaskDescription("Same as sequence but will run the last child regardless of the success or failure on the previous childs.")]
[TaskIcon("{SkinColor}SequenceIcon.png")]
public class SequenceLast : Composite {
    ...
    public override bool CanExecute () {
        ...
        if (currentChildIndex < children.Count) {
            //If the last child run failed and it is not the last one in the branch 
            if (executionStatus == TaskStatus.Failure && currentChildIndex != children.Count - 1) {
